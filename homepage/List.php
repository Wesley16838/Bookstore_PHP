	...
	<div class="container">
		...
		if (!empty($_POST['search'])) {
			...
			try
			{
				...
				while($row = $q->fetch()) 
				{
			echo '<tr> <td>' . '<a href="http://chelan.highline.edu/~wesley16838/cs215/week3/producttest.php?UID='.$row['UID'].'"><img src="data:image/jpeg;base64,' . $row['Image'].'"/></a>'. '</td> <td><a href="http://chelan.highline.edu/~wesley16838/cs215/week3/producttest.php?UID='.$row['UID'].'">'. $row['booktitle']. "</a><br>" .$row['Genre'].",&nbsp".$row['bookauthor'].  "<br>" . "$" . $row['Price'] . "<hr>" . $row['Description']. '</td></tr>';
